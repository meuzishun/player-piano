const player = {
  ...
  addPossibleSequence: function (mode, type) {
    ...
    newSequences.forEach((sequence) =>
      this.possiblePitchSequences.push(sequence)
    );
  },
  ...
  findGestureDuration: function () {
    return (
      this.currentGesture.durationSequence.reduce((a, b) => a + b) *
      1000 *
      this.beatLength
    );
  },
...
class Gesture extends Sequence {
  ...
  determineBeatSeq() {
    return this.pitchSequence.length === 3
      ? [1, 2, 3]
      : this.pitchSequence.length === 4
      ? [1, 2, 2.5, 3]
      : undefined;
  }
