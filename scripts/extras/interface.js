const interface = {
  activateMouse: function () {
    ...
    piano.display.controls.powerBtn.addEventListener(
      'click',
      control.powerToggle
    );
    piano.display.controls.volumeSldr.addEventListener(
      'change',
      control.volumeChange
    );
    piano.display.controls.playBtn.addEventListener(
      'click',
      control.playToggle
    );
    piano.display.controls.recordBtn.addEventListener(
      'click',
      control.recordToggle
    );
    piano.display.controls.keyboardBtn.addEventListener(
      'click',
      control.keyboardToggle
    );
  },
  ...
  box.addEventListener('input', (e) => {
    ...
    piano.display.controls.gestureBoxes.forEach((box) => {
      if (box.checked) {
        ...
        player.possiblePitchSequences.push(
          ...player.totalPitchSequences[box.dataset.mode][box.dataset.gesture]
        );
  ...
  checkInput: function () {
    this.input.forEach((midi, i) => {
      if (midi === player.currentGesture.pitchSequence[i]) {
        ...
        piano.display.feedback.answers[i]
