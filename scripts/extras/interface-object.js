const interface = {
    ...
    keySpecs: function(key, midi) {
        ...
        if (interface.blackPC.includes(pitchClass)) {
            ...
            const keyContainer = interface.keyContainers[interface.keyContainers.length - 1];
            keyContainer.appendChild(key);
        }
    ...
    buildMarkup: function() {
        for (let i = this.startingMidi, endingMidi = this.numberOfKeys + this.startingMidi; i < endingMidi; i++) {
            const key = this.createKey(i);
            interface.keys.push(key);
        }

        interface.CSS_access.style.setProperty('--num_of_keyContainers', this.keyContainers.length);
    },
